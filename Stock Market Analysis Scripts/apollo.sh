clear
echo "WELCOME TO APOLLO CORPORATIONS"
echo "|------------------------------|"
echo""
echo""
select=
...
select volume/sum(volume) from record; 
EOF
;;
3)mysql -u"root" -p"root" "register" <<EOF
select name from regis where company"apollo"; 
EOF
;;
*)echo "nothing here"
esac
