echo "                                                 PRESENT TREND"
echo""
echo "Press 1 to see open and close from selected company"
...
read var
case $var in
    1) echo "Please enter the companies"
       read com
       read comp
       mysql -u"root" -p"root" "market" <<EOF
       select open,close from record where name IN ('$com', '$comp'); 
EOF
	;;
...
select *from record where open-close<2; 
EOF
        ;;
     X) ./main_script.sh ;;
esac		
